...
class XsmmJobRunnerOverlayService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (fullPanel == null && miniBubble == null) {
            accountHandles = intent?.getStringExtra(EXTRA_ACCOUNT_HANDLES)?.split(",")?.filter { it.isNotBlank() }.orEmpty()
            ...
            XsmmJobStatusBridge.update(
                if (accountHandles.size > 1) "Đã chọn ${accountHandles.size} tài khoản - đang xử lý @${accountHandles.first()}..."
                else "Sẵn sàng chạy cho @${accountHandles.firstOrNull().orEmpty()}"
            )
        }
    ...
    private fun appVersionName() = try { packageManager.getPackageInfo(packageName, 0).versionName ?: "" } catch (e: PackageManager.NameNotFoundException) { "" }
    ...
    private fun showFullPanel() {
        ...
        val params = WindowManager.LayoutParams(panelW, WindowManager.LayoutParams.WRAP_CONTENT, overlayType(),
            WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE or WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS,
            PixelFormat.TRANSLUCENT
        ).apply { gravity = Gravity.TOP or Gravity.START; x = ((resources.displayMetrics.widthPixels - panelW) / 2).coerceAtLeast(0); y = dp(120) }
        panelParams = params

        val root = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            setPadding(dp(18), dp(16), dp(18), dp(16))
            background = GradientDrawable().apply { cornerRadius = dp(18).toFloat(); setColor(Color.parseColor("#F00E1611")); setStroke(dp(1.5f), Color.parseColor("#16A34A")) }
        }
        ...
        root.addView(TextView(this).apply { text = "NV"; setTextColor(Color.parseColor("#8A93A6")); textSize = 12f; setPadding(0, dp(4), 0, dp(4)) })
        root.addView(TextView(this).apply { text = "Chưa có nhiệm vụ - đang chờ loại nhiệm vụ để lấy job thật"; setTextColor(Color.parseColor("#C7CBD4")); textSize = 12.5f; setPadding(0, 0, 0, dp(4)) })
        root.addView(divider())
    ...
    private fun showMiniBubble() {
        fullPanel?.let { runCatching { windowManager.removeView(it) } }; fullPanel = null
        val size = dp(52)
        val params = WindowManager.LayoutParams(size, size, overlayType(),
            WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE or WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS,
            PixelFormat.TRANSLUCENT
